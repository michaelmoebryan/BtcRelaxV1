...
class DAO {
	...
	public function customerByBitId($bitid) {
		$result = $this->_mysqli->query(sprintf("SELECT * FROM vwCustomerBitid WHERE BitId = '%s' LIMIT 1 ", 
		$this->_mysqli->real_escape_string($bitid)));
		if($result) {
			$row = $result->fetch_assoc();
