...
-- Schema Ident
-- -----------------------------------------------------
CREATE SCHEMA IF NOT EXISTS `bitghluk_prod_v2` DEFAULT CHARACTER SET utf8 ;
USE `bitghluk_prod_v2` ;
...
CREATE TABLE IF NOT EXISTS `Bookmarks` (
  ...
  `State` ENUM('Preparing', 'Checking', 'Rejected', 'Ready', 'Published', 'Saled', 'Lost', 'PreOrdered') CHARACTER SET 'ucs2' NULL DEFAULT 'Preparing',
...
DELIMITER $$
USE `bitghluk_prod_v2`$$
CREATE DEFINER=`bitghluk_prod_v2`@`%` PROCEDURE `CreateCustomerByBitId`(IN pBitId VARCHAR(50), 
...
USE `bitghluk_prod_v2`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`bitghluk_prod_v2`@`%` SQL SECURITY DEFINER VIEW `vwCustomerBitid` AS select `Customers`.`idCustomer` AS `idCustomer`,`Idetities`.`IdentityKey` AS `BitId` from (`Customers` join `Idetities` on(((`Idetities`.`IdentTypeCode` = 'bitid') and (`Idetities`.`idCustomer` = `Customers`.`idCustomer`))));
...
INSERT INTO `Bookmarks` (`idBookmark`, `CreateDate`, `idOrder`, `Quantity`, `EndDate`, `Latitude`, `Longitude`, `Link`, `Description`, `RegionTitle`, `CustomPrice`, `PriceCurrency`, `AdvertiseTitle`, `UnlockDate`, `State`, `IdDroper`, `TargetAddress`) VALUES (DEFAULT, '2016-03-30 03:41:28', NULL, 1, NULL, 50.4536715, 30.6130949, 'https://fastfen.club/cloud/index.php/s/8WJG6FTpvBWC3cV', NULL, 'Киев, м.Дарница', 100, DEFAULT, '0,25 амфетамина', NULL, 'Published', '1Fk8Q3LWcEaqcfpQp6Zv4jNJwdUMutttmN', '19obTLBfmhyiirETcThD1ydMkWGFLuDaqe');
